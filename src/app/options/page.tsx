...
export default function OptionsPage() {
  return (
    <>
      <Section eyebrow="Комплектация" title="Опции и комплектующие" description="Опции рассчитываются по выбранной модели и совместимости систем.">
          ...
          {pergolaOptions.map((option) => (
            <article key={option.id} className="rounded-3xl border border-stone-200 bg-white p-6">
              <div className="relative h-52 w-full overflow-hidden rounded-2xl">
                <Image src={option.images[0]} alt={option.title} fill sizes="(min-width: 1024px) 33vw, (min-width: 768px) 50vw, 100vw" className="object-cover" />
              </div>
              <p className="mt-5 text-sm uppercase tracking-[0.16em] text-stone-500">{option.category}</p>
              ...
              <p className="mt-4 text-sm font-semibold text-stone-950">Стоимость: {option.priceType === "included" ? "в базовой системе" : option.priceType === "extra" ? "дополнительно" : "по запросу"}</p>
            </article>
          ))}
        </div>
      </Section>
